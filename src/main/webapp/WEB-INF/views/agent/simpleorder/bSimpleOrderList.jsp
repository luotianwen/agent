...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/bsimpleorder/">下单管理列表</a></li>
    <li><a href="${ctx}/bsimpleorder/form">下单管理添加</a></li>
</ul>
<form:form id="searchForm" modelAttribute="simpleOrder" action="${ctx}/bsimpleorder/" method="post"
        ...
        <tr>
            ...
            <td>
                <c:if test="${simpleOrder.state==1}">
                    <a href="${ctx}/bsimpleorder/form?id=${simpleOrder.id}">修改</a>
                    <%--<a href="${ctx}/bsimpleorder/delete?id=${simpleOrder.id}"
                       onclick="return confirmx('确认要删除该下单管理吗？', this.href)">删除</a>--%>
                </c:if>
                <c:if test="${simpleOrder.state==3}">
                    <a href="${ctx}/bsimpleorder/after?id=${simpleOrder.id}"
                ...
                <c:if test="${simpleOrder.afterstate==4||simpleOrder.afterstate==5||simpleOrder.afterstate==7}">
                    <a href="${ctx}/bsimpleorder/listSimpleOrderAfter?orderId=${simpleOrder.orderId}">售后信息</a>
                </c:if>
            </td>
        </tr>
    </c:forEach>
