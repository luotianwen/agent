...
<body>
	...
	<form:form id="searchForm" modelAttribute="simpleOrderAfter" action="${ctx}/msimpleorder/listSimpleOrderAfter" method="post" class="breadcrumb form-search">
			...
			<tr>
			 ...
			 <td>
					...
					<c:if test="${not empty simpleOrderAfter.backaddress && empty simpleOrderAfter.delivernumber }">
						<a href="${ctx}/msimpleorder/courier?id=${simpleOrderAfter.id}&orderId=${simpleOrderAfter.orderId}" onclick="return promptxcourier('填写快递信息',   this.href)">快递信息</a>
					</c:if>
				 <c:if test="${  empty simpleOrderAfter.backaddress  }">
					<a href="${ctx}/msimpleorder/deleteSimpleOrderAfter?id=${simpleOrderAfter.id}" onclick="return confirmx('确认要删除该订单售后吗？', this.href)">删除</a>
				 </c:if>
			 </td>
			</tr>
		</c:forEach>
